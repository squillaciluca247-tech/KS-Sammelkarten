...
function drawImageArea(ctx: CanvasRenderingContext2D, rider: Rider, photo?: HTMLImageElement | null) {
  ...
  if (photo) {
    ...
    // Foto/Hintergrund-Mix: Hintergrund bleibt sichtbar und Foto verschmilzt weicher
    ctx.globalAlpha = Math.max(0.35, Math.min(1, rider.photoBlend / 100));
    ctx.drawImage(photo, px, py, dw, dh);
    ctx.globalAlpha = 1;
    ctx.filter = "none";

    // Farb- und Licht-Overlay verbindet Foto und KI-Hintergrund
    ctx.save();
    ctx.globalCompositeOperation = "soft-light";
    const blendLight = ctx.createLinearGradient(x, y, x + w, y + h);
    blendLight.addColorStop(0, "rgba(255,230,140,.20)");
    blendLight.addColorStop(.45, "rgba(255,255,255,.08)");
    blendLight.addColorStop(1, "rgba(0,0,0,.22)");
    ctx.fillStyle = blendLight;
    ctx.fillRect(x, y, w, h);
    ctx.restore();

    // Hintergrund-Schimmer wird wieder über das Foto gelegt
    ctx.save();
    ctx.globalAlpha = Math.max(0, 1 - rider.photoBlend / 100) * 0.75;
    drawGeneratedBackground(ctx, rider, x, y, w, h);
    ctx.restore();
  } else {
    ctx.fillStyle = "#777";
  ...
  const bottom = ctx.createLinearGradient(x, y+h-230, x, y+h);
  bottom.addColorStop(0, "rgba(5,4,3,0)");
  bottom.addColorStop(.55, "rgba(5,4,3,.55)");
  bottom.addColorStop(1, "rgba(5,4,3,.98)");
  ctx.fillStyle = bottom;
  ctx.fillRect(x, y+h-230, w, 230);

  const topBlend = ctx.createLinearGradient(x, y, x, y+130);
  topBlend.addColorStop(0, "rgba(5,4,3,.82)");
  topBlend.addColorStop(1, "rgba(5,4,3,0)");
